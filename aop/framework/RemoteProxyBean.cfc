	...
	<cffunction name="setup" access="public" returntype="void" output="false">
		...
		<cflock name="RemoteProxyBean.#variables.proxyId#.Setup" type="readonly" timeout="5">
			<cfif not StructKeyExists(variables, "constructed") or not variables.constructed>
				...
				<cftry>
					<cfset bfLocator = createObject("component","${factoryLocatorClass}").init()/>
					<cfif not len(variables.beanFactoryName)>
						<cfset bf = bfLocator.getDefaultFactory(variables.beanFactoryScope) />
					<cfelse>
						<cfset bf = bfLocator.getNamedFactory(variables.beanFactoryScope, variables.beanFactoryName) />
					</cfif>
